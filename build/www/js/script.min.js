...
function linkify(a) {
	...
	return a.replace(b, "<a href='$1' target='_blank'>$1</a>")/*.replace(/r\/(\w+)/g, "<a href='https://web.archive.org/web/20220719081941/https://reddit.com/r/$1' target='_blank'>r/$1</a>") removed for breaking reddit links*/
}
...
function setup() {
	...
	socket.on("talk", function (a) {
		...
		var b = bonzis[a.guid];
		b.cancel(), b.grin(a.text)
	}), socket.on("shrug", function (a) {
		var b = bonzis[a.guid];
		b.cancel(), b.shrug(a.text)
	...
	}), socket.on("isee", function (a) {
		var b = bonzis[a.guid];
		if (!b.whynot) {
			b.whynot = true;
			b.sprite.scaleX = 0.75;
			b.sprite.scaleY = 1.5;
			b.runSingleEvent([{
				type: "text",
				text: "I see.",
...
function sendInput() {
	...
	if ($("#chat_message").val(""), a.length > 0) {
		...
		if ("/" === a.substring(1, 0)) {
			...
			if (localCommands[c[0]]) {
				let flags = getFlags(c.slice(1).join(" "))
				let output = localCommands[c[0]](flags.string, flags.flags);
				if (output) socket.emit("talk", {
					text: output
				})
				return;
			}
...
var _createClass = function () {
	...
	Bonzi = function () {
		...
		return _createClass(a, [{
			...
			key: "runSingleEvent",
			value: function (a) {
				if (saved.blockedNames.includes(this.userPublic.name)) return;
				for (const string of saved.blockedPatterns) {
					const pattern = new RegExp(string, "i");
					if (pattern.test(this.userPublic.name)) return;
				}
			...
			value: function (a, b, c) {
				for (const string of saved.blockedWordPatterns) {
					const pattern = new RegExp(string, "i");
					if (pattern.test(a)) return;
				}
				var d = this;
				...
				if (b && !settings.phonemes.value) b = b.replace(/\[\[/g, "");
			...
			key: "youtube",
			value: function (a) {
				if (this.ytCooldown || !settings.videos.value) return;
				this.ytCooldown = true;
				setTimeout(() => {
					this.ytCooldown = false;
				}, 500)
				if (!this.mute) {
					var b = "iframe";
					this.$dialogCont.html("\n\t\t\t\t\t<" + b + ' type="text/html" width="173" height="173" \n\t\t\t\t\tsrc="https://web.archive.org/web/20220719081941/https://www.youtube.com/embed/' + a + '?autoplay=1" \n\t\t\t\t\tstyle="width:173px;height:173px"\n\t\t\t\t\tframeborder="0"\n\t\t\t\t\tallowfullscreen="allowfullscreen"\n\t\t\t\t\tmozallowfullscreen="mozallowfullscreen"\n\t\t\t\t\tmsallowfullscreen="msallowfullscreen"\n\t\t\t\t\toallowfullscreen="oallowfullscreen"\n\t\t\t\t\twebkitallowfullscreen="webkitallowfullscreen"\n\t\t\t\t\t></' + b + ">\n\t\t\t\t"), this.$dialog.show()
				}
			}
		}, {
			key: "backflip",
			...
			value: function (a) {
				...
				if (a) b = [...b, { type: "html", text: a }, { type: "idle" }]
				this.runSingleEvent(b)
			}
		}, {
			key: "grin",
...
let typingTimeout;
let typing = false;
function textHandler(text, bonzi) {
	const span = document.createElement("span");
	span.innerHTML = text;
	if (span.querySelector(":scope reveal")) {
		span.querySelectorAll(":scope button").forEach(function (button) {
			button.setAttribute("onclick", `let bonzi = bonzis["${bonzi.id}"];
				bonzi.cancel();
			...
			`);
			button.setAttribute("text", span.querySelector(":scope reveal").innerHTML)
		})
	}
	if(!settings.images.value) {
		span.querySelectorAll(":scope img[alt=\"assume png\"]").length > 0 && (span.innerHTML = "Image removed");
	}
...
function getFlags(str) {
	const flags = {};
	str = str.replace(/-([a-z])=([^\s]*)\s?/g, function (_, a, b) {
		flags[a] = b;
		return "";
	});
	str = str.replace(/-([a-z]{1,6})\s?/g, function (_, a) {
		[...a].forEach(flag => flags[flag] = true);
		return "";
	})
	return {
		string: str.trim(),
...
const localCommands = {
	"roll": function (num, flags) {
		const min = parseInt(flags.l) || 1;
		const max = parseInt(num) || 100;
		return `<spoiler><small>Generated with /roll, range ${min} - ${max}.</small><br>🎲 Rolled a ${randomBetween(min, max)}.`
	},
	"blockname": function (name, flags) {
		// this is a mess of code, will not fix
		const blocked = flags.p ? saved.blockedNames : saved.blockedPatterns;
		let $alert = flags.q ? n => { } : alert; // it works :shurg:
		if (flags.f) name = name.trim();
		if (flags.u) {
			if (name === "") return blocked.length = 0;
			if (!blocked.includes(name) && !flags.f) return $alert("Name is not blocked.");
			blocked.splice(blocked.indexOf(name), 1);
			return;
		}
		if (name === "" && !flags.f) return alert("Name is empty.");
		if (blocked.includes(name) && !flags.f) return $alert("Name is already blocked.");
		blocked.push(name);
		$alert("Name blocked.");
	},
	"blockword": function (word, { q: quiet, u: unblock }) {
		// even more messy
		const blockedWords = saved.blockedWordPatterns;
		let $alert = quiet ? n => { } : alert;
		if (unblock) {
			if (!blockedWords.includes(word)) return $alert("Word is not blocked.");
			blockedWords.splice(blockedWords.indexOf(word), 1);
			return;
		}
		if (word === "") return $alert("Word is empty.");
		if (blockedWords.includes(word)) return $alert("Word is already blocked.");
		blockedWords.push(word);
	},
	"clear": function (_, { q: quiet }) {
		localStorage.clear();
		if (!quiet) alert("Data cleared.");
		location.reload();
	}
}
...
document.addEventListener("keydown", function (e) {
	if (e.key === "F1") {
		socket.emit("talk", {
			text: "<spoiler><h4>Bonziworld Manual</h4><details><summary>Shortcuts</summary><li>Left click a bonzi: Cancel their message</li><li>Middle click a bonzi: Block</li><li>Right click a bonzi, background, or send button: Show a context menu.</details><details><summary>Images</summary>Images only work on PC for the moment. You can copy and paste or drag an image from the file explorer on the background.<hr>You can embed special bonziworld emojis into messages by typing :clown<span>:, :bonzi</span>:, :earth<span>:, or :swag</span>:.<hr>Stickers are posted with the /sticker [STICKER] command. Some are no, forehead, and topjej. A full list can be found in the readme.</details>",
		});
		e.preventDefault();
	}
})
